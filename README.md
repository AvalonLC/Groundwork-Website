...
- **Last deployed**: 2026-07-20 — `/demo` now reveals an embedded Google Calendar booking widget after the qualifying form is submitted successfully (hybrid flow: qualify then self-book a real time slot), replacing the plain "we'll follow up" success state. Existing form fields, SendGrid notification, and all other forms (contact, signup) are unchanged. (Same-day earlier deploy — per-user rate cut from $29/mo to $25/mo flat, and per-plan included-user allotments introduced: Growth now includes 5 users, Pro now includes 10, Core unchanged at 1. Prior deploy 2026-07-19 — pricing model overhaul: removed the Solo plan, moved to 3 plans, removed role-based seat pricing. Earlier deploy 2026-07-18 — Trades mega-menu redesign.)
